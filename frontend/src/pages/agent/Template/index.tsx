        ...
        <a
          ...
          onClick={async () => {
            try {
              ...
              await downloadAgentTemplate(
                record.id,
                record.source?.name || `${record.name}.bin`,
              );
              ...
              setDownloadingTemplateId((current) =>
  ...
  const handleSave = async (data: Partial<AgentTemplate>) => {
    try {
      await saveAgentTemplate({
        ...
        ...data,
        sourceId:
          data.sourceId || editingItem?.sourceId || editingItem?.source?.id,
      });
      ...
      <DrawerForm
        ...
        initialValues={
          editingItem
            ? {
                ...editingItem,
                sourceId: editingItem.sourceId || editingItem.source?.id,
              }
            : undefined
        }
      >
