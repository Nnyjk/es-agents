...
public class AuthResource {
    ...
    public Response login(@Valid AuthRecord.LoginRequest request) {
        return Response.ok(authService.login(mapLoginRequest(request))).build();
    }

    // 兼容旧的 UserRecord.Login
    private UserRecord.Login mapLoginRequest(AuthRecord.LoginRequest request) {
        return new UserRecord.Login(request.getUsername(), request.getPassword());
    }
